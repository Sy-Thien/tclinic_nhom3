...
class ExaminationPage extends Component {
    constructor(props) {
        super(props);
        this.state = {
            appointment: props.location?.state?.appointment || null,
    ...
    componentDidMount() {
        ...
        const bookingId = this.props.searchParams.get('bookingId');

        if (!appointment && bookingId) {
            this.loadAppointmentById(bookingId);
        } else if (!appointment && !bookingId) {
            alert('Không tìm thấy thông tin lịch hẹn!');
            this.props.navigate('/doctor-portal/appointments');
        } else if (appointment) {
            this.initAppointmentData(appointment);
        }
    }

    componentDidUpdate(prevProps, prevState) {
        // Handle searchParams change
        const prevBookingId = prevProps.searchParams.get('bookingId');
        const currentBookingId = this.props.searchParams.get('bookingId');
        if (prevBookingId !== currentBookingId && currentBookingId && !this.state.appointment) {
            this.loadAppointmentById(currentBookingId);
        }

        // Handle appointment change
        if (prevState.appointment !== this.state.appointment && this.state.appointment) {
            this.initAppointmentData(this.state.appointment);
        }
    }

    initAppointmentData = (appointment) => {
        const updates = {};
        if (appointment.diagnosis) updates.diagnosis = appointment.diagnosis;
        if (appointment.conclusion) updates.conclusion = appointment.conclusion;
        if (appointment.note) updates.note = appointment.note;
    ...
    loadMedicalHistory = async () => {
        ...
        try {
            ...
            if (response.data.success) {
                this.setState({ medicalHistories: response.data.histories || [] });
            }
        } catch (error) {
    ...
    handleSaveExamination = async () => {
        const { formData, appointment } = this.state;
    ...
    handleOpenPrescription = () => {
        if (!this.state.formData.diagnosis.trim()) {
            alert('Vui lòng nhập chẩn đoán trước khi kê đơn!');
            return;
        }
        this.setState({ showPrescriptionModal: true });
    };
    ...
    render() {
        ...
        const { appointment, loadingAppointment, formData, saving, showPrescriptionModal, showPaymentModal, prescriptionId, medicalHistories } = this.state;
            ...
            <div className={styles.container}>
                ...
                <div className={styles.mainContent}>
                        ...
                        <div className={styles.cardBody}>
                            <div className={styles.appointmentInfo}>
                                <div className={styles.timeSlot}>
                                    ...
                                    <span>{this.formatTime(appointment.appointment_time)} - {this.formatDate(appointment.appointment_date)}</span>
                                </div>
                    ...
                    <div className={styles.actions}>
                        <button
                            ...
                            onClick={this.handleSaveExamination}
                            disabled={saving}
                        >
                ...
                {showPrescriptionModal && (
                    <PrescriptionFormPro
                        ...
                        onClose={() => this.setState({ showPrescriptionModal: false })}
                        onSuccess={(prescriptionData) => {
                            console.log('✅ Prescription saved successfully');
                            if (prescriptionData?.id) {
                                this.setState({ prescriptionId: prescriptionData.id });
                            }
                        }}
                    />
                )}
...
export default withRouter(ExaminationPage);
