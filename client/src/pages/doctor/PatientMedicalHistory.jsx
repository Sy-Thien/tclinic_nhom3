...
// Helper functional component for print (useReactToPrint is hooks-only)
function PrintButton({ contentRef, patient, selectedHistory, className }) {
...
class PatientMedicalHistory extends Component {
    ...
    componentDidUpdate(prevProps) {
        if (prevProps.params.patientId !== this.props.params.patientId) {
            this.fetchPatientHistory();
        }
    }
        ...
        try {
            ...
            if (response.data.success) {
                this.setState({
                    ...
                    selectedHistory: response.data.histories?.length > 0 ? response.data.histories[0] : null
                });
            }
        } catch (error) {
    ...
    render() {
        ...
        const { loading, patient, histories, selectedHistory, viewMode } = this.state;
            ...
            <div className={styles.container}>
                ...
                <div className={styles.mobileToggle}>
                    ...
                    <button
                        ...
                        onClick={() => this.setState({ viewMode: 'detail' })}
                    >
                        Chi tiết
                    </button>
                </div>
                            ...
                            <div className={styles.historyItems}>
                                {histories.map((history, index) => (
                                    <div
                                        ...
                                        onClick={() => this.selectHistory(history)}
                                    >
                                        <div className={styles.historyIndex}>#{histories.length - index}</div>
                                        <div className={styles.historyInfo}>
                                            <div className={styles.historyDate}>
                                                {this.formatDateShort(history.visit_date)}
                                                <span className={styles.historyTime}>
                                                    {this.formatTime(history.visit_time)}
                                                </span>
                                            </div>
                                ...
                                <div className={styles.detailContent}>
                                    ...
                                    <div className={styles.visitInfo}>
                                        <div className={styles.visitDate}>
                                            {this.formatDate(selectedHistory.visit_date)}
                                            {selectedHistory.visit_time && (
                                                <span className={styles.visitTime}>
                                                    lúc {this.formatTime(selectedHistory.visit_time)}
                                                </span>
                                            )}
                                        </div>
                                                    ...
                                                    <div className={styles.prescriptionBox}>
                                                        ...
                                                        {selectedHistory.prescription.re_exam_date && (
                                                            <div className={styles.reExamDate}>
                                                                ...
                                                                {this.formatDateShort(selectedHistory.prescription.re_exam_date)}
                                                            </div>
                                                        )}
                                                    </div>
                                                ) : (
