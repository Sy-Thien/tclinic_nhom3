const { Booking, Patient, Specialty, MedicalHistory, Prescription, DoctorSchedule, Service } = require('../Database/Entity');
...
class DoctorAppointmentController {
        ...
        try {
            ...
            const schedules = await DoctorSchedule.findAll({
                where: {
                    ...
                    // Chỉ hiển thị lịch đã được duyệt và đang hoạt động
                },
                order: [
                    ['day_of_week', 'ASC']
                ]
    ...
    // Doctor - Cập nhật trạng thái và thêm chẩn đoán
        async updateBookingDiagnosis(req, res) {
        try {
            const { id } = req.params;
    ...
    // Doctor - Hoàn thành khám
        ...
        try {
            ...
            // ✅ CHỐT CHẶN: Chỉ được hoàn thành/khám khi tới gần giờ (trước 30 phút)
            const today = new Date();
            const todayMidnight = new Date();
            todayMidnight.setHours(0, 0, 0, 0);
...
module.exports = new DoctorAppointmentController();
