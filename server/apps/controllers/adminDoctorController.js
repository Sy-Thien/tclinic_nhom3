const { Doctor, Specialty, DoctorSchedule, TimeSlot, Booking, Review, Prescription, PrescriptionDetail, MedicalHistory, Invoice, InvoiceItem, ConsultationRequest } = require('../Database/Entity');
...
class AdminDoctorController {
    constructor() {
        this.WEEK_DAYS = ['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7', 'Chủ nhật'];
        this.createDoctor = this.createDoctor.bind(this);
    }

    // Hàm tạo lịch làm việc mặc định cho bác sĩ mới
    async createDefaultScheduleForDoctor(doctorId) {
        const schedules = [];

        for (const day of this.WEEK_DAYS) {
    ...
    async createDoctor(req, res) {
        try {
            ...
            try {
                await this.createDefaultScheduleForDoctor(doctor.id);
            } catch (scheduleError) {
...
module.exports = new AdminDoctorController();
