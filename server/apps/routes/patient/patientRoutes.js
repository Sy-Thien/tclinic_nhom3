...
const { Booking, Service, Specialty } = require('../../Database/Entity');
...
router.put('/my-appointments/:id/reschedule', verifyToken, checkPatientRole, async (req, res) => {
    try {
        ...
        if (selectedDate.toDateString() === today.toDateString()) {
            ...
            if (appointmentMinutes < currentMinutes) {
                return res.status(400).json({
                    message: `Không thể đổi sang giờ ${new_time} (đã qua). Vui lòng chọn khung giờ khác.`
                });
            }
        }
        ...
        if (appointment.doctor_id) {
            ...
            const [appH, appM] = new_time.split(':').map(Number);
            ...
            if (appMins < stMins || appMins >= enMins) {
                return res.status(400).json({ message: 'Giờ đăng ký đổi không nằm trong ca làm việc của bác sĩ' });
            }
