const { DoctorSchedule, Doctor, Specialty } = require('../Database/Entity');
...
class AdminDoctorScheduleController {
    ...
    // Lấy lịch làm việc của một bác sĩ
        async getScheduleByDoctor(req, res) {
        try {
    ...
    // Xóa tất cả lịch của một bác sĩ
        async deleteAllSchedulesForDoctor(req, res) {
        try {
            const { doctorId } = req.params;
    ...
    // Lấy danh sách lịch chờ phê duyệt
        async getPendingSchedules(req, res) {
        try {
    ...
    // Tự động tạo và duyệt lịch cả tuần cho bác sĩ chưa đăng ký lịch
        async autoApproveUnregistered(req, res) {
        try {
            const adminId = req.user.id;
            ...
            // Lấy danh sách bác sĩ đã có lịch (bất kỳ ngày nào)
            const existingSchedules = await DoctorSchedule.findAll({
                attributes: ['doctor_id'],
                group: ['doctor_id']
            });
            const doctorsWithSchedule = new Set(existingSchedules.map(s => s.doctor_id));

            // Lọc bác sĩ chưa có lịch nào
            const unregisteredDoctors = doctors.filter(d => !doctorsWithSchedule.has(d.id));

            if (unregisteredDoctors.length === 0) {
                return res.json({
                    success: true,
                    message: 'Tất cả bác sĩ đã có lịch làm việc. Không cần tự động duyệt.',
                    created: 0,
                    doctors: []
                });
            }
            ...
            for (const doctor of unregisteredDoctors) {
                const created = [];
                ...
                results.push({ doctor_id: doctor.id, name: doctor.full_name, schedules_created: created.length });
                console.log(`✅ Auto-approved full week for doctor ${doctor.id} (${doctor.full_name})`);
            }

            res.json({
                success: true,
                message: `Đã tự động duyệt lịch cả tuần cho ${results.length} bác sĩ chưa đăng ký`,
                created: results.reduce((sum, r) => sum + r.schedules_created, 0),
                doctors: results
            });
        } catch (error) {
            console.error('❌ Error auto-approving unregistered doctors:', error);
            res.status(500).json({
                success: false,
                message: 'Lỗi tự động duyệt lịch',
                error: error.message
            });
        }
    };
...
module.exports = new AdminDoctorScheduleController();
