const { TimeSlot, Doctor, Room, Booking, Specialty } = require('../models');
...
exports.createMultipleTimeSlots = async (req, res) => {
    try {
...
exports.incrementPatientCount = async (timeSlotId) => {
    try {
...
exports.decrementPatientCount = async (timeSlotId) => {
    try {
        const timeSlot = await TimeSlot.findByPk(timeSlotId);
        if (!timeSlot) {
            throw new Error('Time slot không tồn tại');
