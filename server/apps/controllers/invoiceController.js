const { Invoice, InvoiceItem, Booking, Patient, Doctor, Service, Prescription, PrescriptionDetail, Drug } = require('../Database/Entity');
...
class InvoiceController {
    ...
    async createInvoice(req, res) {
        try {
            ...
            const invoice_code = await this.generateInvoiceCode();
    ...
    async updatePaymentStatus(req, res) {
        try {
            ...
            // ⏰ GUARD: Chỉ auto-complete nếu đã đến ngày/giờ khám (hoặc trước 30 phút)
            if (payment_status === 'paid' && invoice.booking_id) {
                ...
                if (booking && booking.status !== 'completed') {
                    ...
                    if (apptDate < todayMidnight) {
                        // Ngày đã qua → cho phép complete
                        allowComplete = true;
                    } else if (apptDate.getTime() === todayMidnight.getTime()) {
                        // Hôm nay → kiểm tra giờ (cho phép trước 30 phút)
                        if (booking.appointment_time) {
                            const [h, m] = booking.appointment_time.split(':').map(Number);
                            const apptMins = h * 60 + m;
                            const currentMins = now.getHours() * 60 + now.getMinutes();
                            allowComplete = currentMins >= apptMins - 30;
                        } else {
                            allowComplete = true;
                        }
                    }
                    // apptDate > todayMidnight → lịch tương lai → KHÔNG complete

                    if (allowComplete) {
                        await booking.update({ status: 'completed' });
                        console.log(`✅ Booking ${booking.booking_code} marked as completed after payment`);
                    } else {
                        console.log(`⚠️ Booking ${booking.booking_code} NOT auto-completed: appointment is in the future`);
                    }
                }
            }
