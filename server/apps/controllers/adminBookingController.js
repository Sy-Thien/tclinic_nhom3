const { Booking, Patient, Doctor, Specialty, DoctorSchedule } = require('../Database/Entity');
...
class AdminBookingController {
        ...
        try {
            ...
            // Khi lọc theo ngày cụ thể: sắp xếp theo giờ khám trong ngày
            // Mặc định: ngày khám MỚI NHẤT lên đầu (DESC), cùng ngày thì theo giờ tăng dần (ASC)
            // Tách thành 2 query riêng để tránh bug Sequelize subquery phá vỡ ORDER BY
            const order = date
                ? [['appointment_date', 'ASC'], ['appointment_time', 'ASC']]
                : [['appointment_date', 'DESC'], ['appointment_time', 'ASC']];

            // Query 1: đếm tổng (không cần include, nhanh hơn)
            const count = await Booking.count({ where });

            // Query 2: lấy dữ liệu với phân trang và sắp xếp đúng
    ...
    // Admin - Cập nhật booking
        ...
        try {
            ...
            // ⏰ GUARD: Không cho phép set completed nếu chưa đến ngày/giờ khám
            if (updateData.status === 'completed') {
                const now = new Date();
                const todayMidnight = new Date();
                todayMidnight.setHours(0, 0, 0, 0);

                const apptDateStr = booking.appointment_date instanceof Date
                ...
                if (apptDate > todayMidnight) {
                    return res.status(400).json({ message: 'Không thể hoàn thành lịch hẹn chưa đến ngày khám' });
                }

                if (apptDate.getTime() === todayMidnight.getTime() && booking.appointment_time) {
                    const [h, m] = booking.appointment_time.split(':').map(Number);
                    const apptMins = h * 60 + m;
                    const currentMins = now.getHours() * 60 + now.getMinutes();
                    if (currentMins < apptMins - 30) {
                        const allowH = Math.floor((apptMins - 30) / 60);
                        const allowM = String((apptMins - 30) % 60).padStart(2, '0');
                        return res.status(400).json({ message: `Chưa tới giờ khám. Chỉ có thể hoàn thành trước 30 phút (từ ${allowH}:${allowM})` });
                    }
                }
            }
    ...
    // Admin - Lấy danh sách bác sĩ khả dụng cho booking (cùng chuyên khoa)
        async getAvailableDoctorsForBooking(req, res) {
        try {
            const { id } = req.params;
    ...
    // Admin - Lấy danh sách bác sĩ còn trống cho booking (để gán)
        async getAvailableDoctorsForAssignment(req, res) {
        try {
    ...
    // Admin - Gán bác sĩ cho booking
        async assignDoctorToBooking(req, res) {
        try {
            const { booking_id } = req.params;
            const { doctor_id } = req.body;
