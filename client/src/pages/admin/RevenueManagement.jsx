...
class RevenueManagement extends Component {
    ...
    fetchDashboardStats = async () => {
        try {
            const todayResponse = await api.get('/api/patient/invoices/stats/revenue', {
                params: { period: 'today' }
            });
            const monthResponse = await api.get('/api/patient/invoices/stats/revenue', {
                params: { period: 'month' }
            });
    ...
    fetchRevenueStats = async () => {
        try {
            ...
            const response = await api.get('/api/patient/invoices/stats/revenue', {
                params: { period: this.state.period }
            });
    ...
    fetchInvoices = async () => {
        try {
            const response = await api.get('/api/patient/invoices', {
                params: { limit: 50 }
            });
            this.setState({
    ...
    render() {
        const { loading, period, year, month, stats, dashboardStats, invoices, showInvoiceList } = this.state;
            ...
            <div className={styles.container}>
                ...
                <div className={styles.dashboardCards}>
                    <div className={`${styles.card} ${styles.cardToday}`}>
                        ...
                        <div className={styles.cardContent}>
                            ...
                            <span className={styles.cardValue}>{this.formatCurrency(dashboardStats?.todayRevenue)}</span>
                        </div>
                    </div>
                    <div className={`${styles.card} ${styles.cardMonth}`}>
                        ...
                        <div className={styles.cardContent}>
                            ...
                            <span className={styles.cardValue}>{this.formatCurrency(dashboardStats?.monthRevenue)}</span>
                        </div>
                    </div>
                ...
                <div className={styles.filters}>
                    ...
                    {period !== 'today' && (
                        <div className={styles.filterGroup}>
                            <label>Năm:</label>
                            <select value={year} onChange={(e) => this.setState({ year: Number(e.target.value) })}>
                    ...
                    {period === 'month' && (
                        <div className={styles.filterGroup}>
                            <label>Tháng:</label>
                            <select value={month} onChange={(e) => this.setState({ month: Number(e.target.value) })}>
                    ...
                    <button className={styles.viewListBtn} onClick={this.fetchInvoices}>
                        ...
                        <div className={styles.summaryGrid}>
                            ...
                            <div className={styles.summaryItem}>
                                ...
                                <span className={styles.summaryValue}>
                                    {this.formatCurrency(stats.pendingAmount)}
                                </span>
                            </div>
                            <div className={styles.summaryItem}>
                                    ...
                                    {stats.byPaymentMethod.map((method, idx) => (
                                        <div key={idx} className={`${styles.methodCard} ${styles[method.payment_method]}`}>
                                            ...
                                            <div className={styles.methodInfo}>
                                                ...
                                                <span className={styles.methodTotal}>{this.formatCurrency(method.total)}</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className={styles.chart}>
                                <div className={styles.chartBars}>
                                    {stats.dailyRevenue.map((item, index) => {
                                        const maxVal = this.getMaxValue(stats.dailyRevenue);
                                        ...
                                        return (
                                            <div key={index} className={styles.barContainer}>
                                                <div
                                                    ...
                                                    title={this.formatCurrency(item.total)}
                                                >
                                                    <span className={styles.barValue}>
                                                        {Number(item.total) > 0 ? this.formatCurrency(item.total) : ''}
                                                    </span>
                                                </div>
                ...
                {/* Invoice List Modal */}
                {showInvoiceList && (
                    <div className={styles.modalOverlay} onClick={() => this.setState({ showInvoiceList: false })}>
                        <div className={styles.modal} onClick={e => e.stopPropagation()}>
                            <div className={styles.modalHeader}>
                                <h2>📋 Danh sách hóa đơn</h2>
                                <button onClick={() => this.setState({ showInvoiceList: false })}>✕</button>
                            </div>
                                    ...
                                    <tbody>
                                        {invoices.map(inv => (
                                            <tr key={inv.id}>
                                                ...
                                                <td>{this.formatCurrency(inv.service_fee)}</td>
                                                <td>{this.formatCurrency(inv.drug_fee)}</td>
                                                <td className={styles.totalCell}>{this.formatCurrency(inv.total_amount)}</td>
                                                <td>
