const { TimeSlot, Doctor, Room, Specialty, Booking, Patient, DoctorSchedule } = require('../Database/Entity');
const { Op } = require('sequelize');

// Lấy danh sách time slots

class AdminTimeSlotController {
    ...
    // Tạo nhiều time slots cùng lúc (bulk create)
        async bulkCreateTimeSlots(req, res) {
        try {
    ...
    // Lấy thống kê time slots
        async getTimeSlotStats(req, res) {
        try {
    ...
    // Xóa nhiều time slots cùng lúc
        async bulkDeleteTimeSlots(req, res) {
        try {
...
module.exports = new AdminTimeSlotController();
