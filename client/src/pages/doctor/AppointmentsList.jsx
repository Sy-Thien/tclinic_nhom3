...
class AppointmentsList extends Component {
    ...
    fetchAppointments = async () => {
        const { selectedDate, viewMode, filterStatus, searchTerm } = this.state;
        try {
            ...
            if (viewMode === 'week') {
                const weekRange = this.getWeekRange(selectedDate);
            ...
            this.setState({ appointments: fetchedAppointments });
        } catch (error) {
    ...
    handleStartExam = (appointment) => {
        this.props.navigate(`/doctor-portal/examination?bookingId=${appointment.id}`);
    };

    handleViewDetail = (appointment) => {
        this.setState({ selectedAppointment: appointment, showDetailModal: true });
    };

    formatTime = (time) => time ? time.substring(0, 5) : '';
    ...
    getAppointmentsForDate = (dateStr) => {
        return this.state.appointments.filter(apt => apt.appointment_date === dateStr);
    };

    changeWeek = (direction) => {
        const newDate = new Date(this.state.selectedDate);
        newDate.setDate(newDate.getDate() + (direction * 7));
        this.setState({ selectedDate: newDate.toISOString().split('T')[0] });
    };
    ...
    render() {
        ...
        const { loading, appointments, selectedDate, viewMode, filterStatus, searchTerm, selectedAppointment, showDetailModal } = this.state;
            ...
            <div className={styles.container}>
                    ...
                    <div className={styles.headerActions}>
                        <div className={styles.viewToggle}>
                            ...
                            <button
                                ...
                                onClick={() => this.setState({ viewMode: 'week' })}
                                title="Xem theo tuần"
                            >
                                📊 Tuần
                            </button>
                ...
                <div className={styles.controlsBar}>
                    <div className={styles.searchBox}>
                        ...
                        />
                        {searchTerm && (
                            <button onClick={() => this.setState({ searchTerm: '' })} className={styles.clearBtn}>
                        ...
                        <div className={styles.dateControls}>
                            {viewMode === 'week' && (
                                <div className={styles.weekNav}>
                                    <button onClick={() => this.changeWeek(-1)} title="Tuần trước">◀</button>
                                    <span>{this.getWeekRange(selectedDate).start} - {this.getWeekRange(selectedDate).end}</span>
                                    <button onClick={() => this.changeWeek(1)} title="Tuần sau">▶</button>
                                </div>
                            )}
                            {viewMode === 'day' && (
                                <input
                            ...
                            <button
                                onClick={() => this.setState({ selectedDate: new Date().toISOString().split('T')[0] })}
                ...
                <div className={styles.filters}>
                    ...
                    <button
                        ...
                        onClick={() => this.setState({ filterStatus: 'waiting_doctor_confirmation' })}
                    >
                        Chờ xác nhận
                    </button>
                    <button
                        className={filterStatus === 'confirmed' ? styles.active : ''}
                        onClick={() => this.setState({ filterStatus: 'confirmed' })}
                    >
                        Đã xác nhận
                    </button>
                    <button
                ...
                {loading ? (
                            ...
                            <tbody>
                                {appointments.map(apt => (
                                    <tr key={apt.id} className={styles.tableRow}>
                                        ...
                                        <td data-label="Trạng thái">{this.getStatusBadge(apt.status)}</td>
                                        <td data-label="Thao tác" className={styles.actionsCell}>
                                            <div className={styles.actionButtons}>
                                                <button
                                                    className={styles.btnViewSmall}
                                                    onClick={() => this.handleViewDetail(apt)}
                                                    title="Xem chi tiết"
                                                >
                                                    Xem
                                                </button>
                                                {apt.status === 'waiting_doctor_confirmation' && (
                                                    <>
                                                        <button
                                                            onClick={() => this.handleConfirm(apt.id)}
                                                        ...
                                                        <button
                                                            onClick={() => this.handleReject(apt.id)}
                                                ...
                                                {apt.status === 'confirmed' && (
                                                    <button
                                                        onClick={() => this.handleStartExam(apt)}
                    ...
                    <div className={styles.weekView}>
                        {this.getWeekDays().map(day => {
                            const dayAppointments = this.getAppointmentsForDate(day.date);
                                        ...
                                        {dayAppointments.length === 0 ? (
                                            ...
                                            dayAppointments.map(apt => (
                                                <div key={apt.id} className={styles.weekAppointmentCard}>
                                                    <div className={styles.aptTime}>{this.formatTime(apt.appointment_time)}</div>
                                                    <div className={styles.aptPatient}>{apt.patient_name || apt.patient?.full_name || 'N/A'}</div>
                                                    <div className={styles.aptStatus}>{this.getStatusBadge(apt.status)}</div>
                                                    <div className={styles.aptActions}>
                                                        {apt.status === 'waiting_doctor_confirmation' && (
                                                            <>
                                                                <button onClick={() => this.handleConfirm(apt.id)} className={styles.btnConfirm}>Duyệt</button>
                                                                <button onClick={() => this.handleReject(apt.id)} className={styles.btnReject}>Từ chối</button>
                                                            </>
                                                        )}
                                                        {apt.status === 'confirmed' && (
                                                            <button onClick={() => this.handleStartExam(apt)} className={styles.btnExam}>Khám</button>
                                                        )}
                                                        <button onClick={() => this.handleViewDetail(apt)} className={styles.btnDetail}>Xem</button>
                                                    </div>
                                                </div>
                                            ))
                                        )}
                                ...
                                <div className={styles.modalSection}>
                                    ...
                                    <div className={styles.infoGrid}>
                                        <div className={styles.infoItem}>
                                            ...
                                            <span className={styles.value}>{this.formatDate(selectedAppointment.appointment_date)}</span>
                                        </div>
                                        <div className={styles.infoItem}>
                                            <span className={styles.label}>Giờ khám:</span>
                                            <span className={styles.value}>{this.formatTime(selectedAppointment.appointment_time)}</span>
                                        </div>
                                        <div className={styles.infoItem}>
                                            <span className={styles.label}>Trạng thái:</span>
                                            <span className={styles.value}>{this.getStatusBadge(selectedAppointment.status)}</span>
                                        </div>
                                    </div>
                                </div>
