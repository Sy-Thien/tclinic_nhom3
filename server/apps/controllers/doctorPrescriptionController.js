const { Prescription, PrescriptionDetail, Drug, Booking, Doctor, Patient } = require('../Database/Entity');
...
class DoctorPrescriptionController {
        ...
        try {
            ...
            // ✅ CHỐT CHẶN: Chỉ được kê đơn khi tới gần giờ (trước 30 phút)
            ...
            if (appointmentDate > todayMidnight) {
                return res.status(400).json({ success: false, message: 'Chưa đến ngày hẹn, không thể kê đơn' });
            }
            ...
            if (appointmentDate.getTime() === todayMidnight.getTime() && booking.appointment_time) {
                ...
                if (currentMinutes < appointmentMinutes - 30) {
                    ...
                    return res.status(400).json({ success: false, message: `Chưa tới giờ khám. Chỉ có thể kê đơn trước 30 phút (từ ${allowH}:${allowM})` });
                }
            }
    ...
    // ✅ GET - Đơn thuốc của lịch khám cụ thể
        async getPrescriptionByBookingId(req, res) {
        try {
...
module.exports = new DoctorPrescriptionController();
