const { DoctorSchedule, Doctor, Booking, TimeSlot } = require('../Database/Entity');
...
class DoctorScheduleController {
    ...
    // Lấy tất cả bác sĩ với lịch làm việc
        async getAllDoctorsWithSchedule(req, res) {
        try {
            const doctors = await Doctor.findAll({
                include: [
                    ...
                    { model: require('../Database/Entity').Specialty, as: 'specialty' }
                ]
            });
            res.json(doctors);
        } catch (error) {
    ...
    // ✅ NEW: Lấy time slots của bác sĩ theo ngày với số lượng booking
        async getDoctorTimeSlotsWithBookings(req, res) {
        try {
            const { doctorId } = req.params;
